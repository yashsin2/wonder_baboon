...
function renderGreeting() {
    ...
    if (token && user) {
        ...
        if (titleEl)
            titleEl.textContent = `Hi, ${displayName.split(" ")[0]}`;
        if (greetingEl)
            greetingEl.textContent = user.email || "Welcome back!";
    }
    else {
        if (titleEl)
            titleEl.textContent = "Wonder Baboon";
        if (greetingEl)
            greetingEl.textContent = "Sign in to manage your trips";
    }
}
...
function highlightActiveNav() {
    ...
    document.querySelectorAll("[data-panel-nav] .up-nav-item").forEach((a) => {
