...
function resolveItineraryId(title, location = "") {
...
export function tripHasItinerary(title, location = "") {
 ...
 */
export function getItineraryHtml(title, location = "") {
    const id = resolveItineraryId(title, location);
    if (id === "jibhi")
        return JIBHI_HTML;
    if (id === "chandratal")
        return CHANDRATAL_HTML;
    return null;
}
export function tripHasItineraryForTrip(trip) {
    if ((trip.itineraryHtml || "").trim().length > 0)
        return true;
    return tripHasItinerary(trip.title, trip.location || "");
}
export function getItineraryHtmlForTrip(trip) {
