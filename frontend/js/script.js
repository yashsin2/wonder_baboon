 ...
 */
function heroSlideImageUrl(slide) {
    const match = slide.style.backgroundImage.match(/url\(["']?([^"')]+)/i);
    return match?.[1] ?? null;
}
async function preloadHeroSlides(slides) {
...
function setupHeroSlideshow() {
    ...
    const onLeaveEnd = (event) => {
        if (event.propertyName !== "opacity")
            return;
        finishLeave(event.currentTarget);
    };
    const crossfadeTo = (nextIndex) => {
        ...
        window.setTimeout(() => {
            if (prev.classList.contains("is-leaving"))
                finishLeave(prev);
        }, 800);
    };
    const advance = () => {
