...
import {
  ...
  getGalleryHeroImage,
  getGalleryMapPin,
  loadTravelGalleryFromApi,
  PREVIOUS_ADVENTURES,
  type AdventureDestination,
  type AdventureTestimonial,
} from "./previous-adventures-data.js";
...
let activeAdventureId: string | null = null;
let lightboxPhotos: string[] = [];
let lightboxIndex = 0;
let indiaSvg: SVGSVGElement | null = null;
let reviewPageIndex = 0;
let reviewPageCount = 0;
let reviewBookOpen = false;

function statItem(icon: string, label: string): string {
  return `
...
function renderReviewBook(testimonials: AdventureTestimonial[]): string {
...
function renderDetailContent(adv: AdventureDestination): string {
  const stats: string[] = [];
  if (adv.dates) stats.push(statItem("📅", adv.dates));
  if (adv.travelers) stats.push(statItem("👥", `${adv.travelers} Travelers`));
  if (adv.days) stats.push(statItem("🕐", `${adv.days} Days`));
  if (adv.photoCountLabel) stats.push(statItem("📷", `${adv.photoCountLabel} Photos`));

  const highlights =
    adv.highlights && adv.highlights.length
...
function renderComingSoon(adv: AdventureDestination): string {
  return `
    <header class="adv-detail-head">
      <h2 class="adv-detail-title" id="advDetailTitle">${escapeHtml(adv.title)}</h2>
    </header>
...
function pinSubtitle(adv: AdventureDestination): string {
  if (adv.pinSubtitle) return adv.pinSubtitle;
  return adv.status === "completed" ? "Completed" : "Coming Soon";
}

const PIN_SHORT_NAMES: Record<string, string> = {
  himachal: "Himachal",
...
const PIN_LABEL_PLACEMENT: Record<string, string> = {
...
function pinShortState(adv: AdventureDestination): string {
  return PIN_SHORT_NAMES[adv.id] ?? adv.state;
}

function setPinActive(id: string | null): void {
  document.querySelectorAll<HTMLButtonElement>(".adv-map-pin, .adv-pin-chip").forEach((el) => {
...
function drawPinToCardLine(activeId: string | null): void {
  if (!isDesktopLayout()) {
...
function openDetail(adv: AdventureDestination): void {
  ...
  panel.hidden = false;
  if (!isDesktopLayout()) backdrop?.removeAttribute("hidden");
...
function setReviewPage(index: number): void {
  if (reviewPageCount < 1) return;
  reviewPageIndex = Math.max(0, Math.min(index, reviewPageCount - 1));

  const track = document.getElementById("advReviewTrack");
  const leaf = document.getElementById("advReviewLeaf");
  if (track) track.style.transform = `translateX(-${reviewPageIndex * 100}%)`;

  const counter = document.getElementById("advReviewCounter");
  if (counter) counter.textContent = `${reviewPageIndex + 1} / ${reviewPageCount}`;
...
function bindReviewBook(): void {
  ...
  const goNext = (e?: Event) => {
    e?.stopPropagation();
    if (reviewPageIndex >= reviewPageCount - 1) return;
    setReviewPage(reviewPageIndex + 1);
  };
  ...
  document.getElementById("advReviewLeaf")?.addEventListener("click", (e) => {
    if (!reviewBookOpen) return;
    if ((e.target as HTMLElement).closest("button")) return;
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    const x = e.clientX - rect.left;
    if (x < rect.width * 0.32) goPrev(e);
    else goNext(e);
  });
  ...
  book?.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape") closeReviewBook();
  });
...
function bindDetailEvents(adv: AdventureDestination): void {
  bindReviewBook();

  document.getElementById("advViewAllPhotos")?.addEventListener("click", () => {
    if (adv.photos.length) openLightbox(adv.photos, 0);
  });

  document.getElementById("advWatchReel")?.addEventListener("click", () => {
    if (adv.videos?.length) {
      document.querySelector<HTMLVideoElement>(".adv-video-card video")?.play();
      return;
    }
    if (adv.photos.length) openLightbox(adv.photos, 0);
  });
...
function openLightbox(photos: string[], start: number): void {
...
function updateLightbox(): void {
  ...
  const img = lb?.querySelector<HTMLImageElement>(".adv-lightbox-img");
...
function onLightboxKey(event: KeyboardEvent): void {
  if (!document.body.classList.contains("adv-lightbox-open")) return;
...
function renderMapPins(): void {
  ...
  wrap.querySelectorAll<HTMLButtonElement>(".adv-map-pin").forEach((pin) => {
...
function renderPinChips(): void {
  ...
  nav.querySelectorAll<HTMLButtonElement>(".adv-pin-chip").forEach((chip) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  void (async () => {
    ...
    initDefaultView();
    await initMap().then(() => {
      if (activeAdventureId) drawPinToCardLine(activeAdventureId);
    });
  })();
});
...
function bindChrome(): void {
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && document.body.classList.contains("adv-detail-open")) {
      if (document.body.classList.contains("adv-lightbox-open")) closeLightbox();
      else closeDetail();
    }
  });
}
...
async function initMap(): Promise<void> {
  ...
  try {
    ...
    indiaSvg?.querySelectorAll<SVGPathElement>(".adv-india-state").forEach((path) => {
    ...
    if (loading) loading.textContent = "Map could not load. Use the destination buttons below.";
  }
}
