...
type GalleryTestimonial = { name: string; quote: string; avatar?: string };
type GalleryHighlight = { title: string; image: string };
...
let galleryDestinations: GalleryDestination[] = [];
let activeGalleryId = "";
...
async function fetchGalleryList(authHeaders: Record<string, string>): Promise<GalleryDestination[]> {
  const res = await fetch(`${API_BASE_URL}/admin/travel-gallery`, { headers: authHeaders });
  if (!res.ok) throw new Error(await parseError(res));
  const data = (await res.json()) as { destinations?: GalleryDestination[] };
...
function readTestimonialsFromDom(): GalleryTestimonial[] {
  const root = getEditorRoot();
  if (!root) return [];
  return Array.from(root.querySelectorAll<HTMLElement>("[data-gallery-review]"))
    .map((row) => ({
      name: row.querySelector<HTMLInputElement>("[data-review-name]")?.value.trim() ?? "",
      quote: row.querySelector<HTMLTextAreaElement>("[data-review-quote]")?.value.trim() ?? "",
    }))
    .filter((t) => t.name && t.quote);
}

function readHighlightsFromDom(): GalleryHighlight[] {
  const root = getEditorRoot();
  if (!root) return [];
  return Array.from(root.querySelectorAll<HTMLElement>("[data-gallery-highlight]"))
    .map((row) => ({
      title: row.querySelector<HTMLInputElement>("[data-highlight-title]")?.value.trim() ?? "",
      image: row.querySelector<HTMLInputElement>("[data-highlight-image]")?.value.trim() ?? "",
    }))
    .filter((h) => h.title && h.image);
}

function readPhotosFromDom(): string[] {
  const raw = (document.getElementById("galleryPhotos") as HTMLTextAreaElement | null)?.value ?? "";
  return raw
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean);
...
function renderReviewRow(t: GalleryTestimonial, index: number): string {
  return `
...
function renderHighlightRow(h: GalleryHighlight, index: number): string {
  return `
...
function renderGalleryEditor(dest: GalleryDestination): void {
  const root = getEditorRoot();
  if (!root) return;
...
function authHeadersFromRoot(): Record<string, string> {
...
let boundAuthHeaders: Record<string, string> = {};

function bindGalleryEditorEvents(destId: string, authHeaders: Record<string, string>): void {
...
function bindRemoveReviewButtons(): void {
  document.querySelectorAll<HTMLButtonElement>("[data-remove-review]").forEach((btn) => {
    btn.onclick = () => btn.closest("[data-gallery-review]")?.remove();
  });
}

function bindRemoveHighlightButtons(): void {
  document.querySelectorAll<HTMLButtonElement>("[data-remove-highlight]").forEach((btn) => {
    btn.onclick = () => btn.closest("[data-gallery-highlight]")?.remove();
  });
}

async function uploadGalleryPhoto(destId: string): Promise<void> {
  const input = document.getElementById("galleryPhotoFile") as HTMLInputElement | null;
  ...
  try {
    ...
    if (!res.ok) throw new Error(await parseError(res));
    const data = (await res.json()) as { path?: string; destination?: GalleryDestination };
    const photosEl = document.getElementById("galleryPhotos") as HTMLTextAreaElement | null;
    ...
    if (data.destination) {
      ...
      if (idx >= 0) galleryDestinations[idx] = data.destination;
    }
    if (status) status.textContent = "Photo uploaded. Click Save to publish other edits.";
...
async function saveGalleryDestination(destId: string): Promise<void> {
  const status = document.getElementById("gallerySaveStatus");
  const btn = document.getElementById("gallerySaveBtn") as HTMLButtonElement | null;

  const body = {
    status: (document.getElementById("galleryStatus") as HTMLSelectElement).value,
    pin_subtitle: (document.getElementById("galleryPinSubtitle") as HTMLInputElement).value.trim(),
    dates: (document.getElementById("galleryDates") as HTMLInputElement).value.trim(),
    travelers: Number((document.getElementById("galleryTravelers") as HTMLInputElement).value) || 0,
    days: Number((document.getElementById("galleryDays") as HTMLInputElement).value) || 0,
    photo_count_label: (document.getElementById("galleryPhotoCount") as HTMLInputElement).value.trim(),
    hero_image: (document.getElementById("galleryHero") as HTMLInputElement).value.trim(),
    story: (document.getElementById("galleryStory") as HTMLTextAreaElement).value.trim(),
  ...
  try {
    ...
    if (!res.ok) throw new Error(await parseError(res));
    const data = (await res.json()) as { destination?: GalleryDestination };
    if (data.destination) {
      const idx = galleryDestinations.findIndex((d) => d.id === destId);
      if (idx >= 0) galleryDestinations[idx] = data.destination;
    }
    if (status) status.textContent = "Saved — live on Travel Gallery page.";
  } catch (err) {
...
function populateGallerySelect(): void {
  const select = document.getElementById("galleryDestSelect") as HTMLSelectElement | null;
  if (!select) return;
  select.innerHTML = galleryDestinations
    .map(
      (d) =>
        `<option value="${escapeHtml(d.id)}" ${d.id === activeGalleryId ? "selected" : ""}>${escapeHtml(d.state)} (${escapeHtml(d.status === "completed" ? "completed" : "coming soon")})</option>`
    )
    .join("");
}

function selectGalleryDestination(id: string): void {
  activeGalleryId = id;
...
export async function loadAdminGalleryTab(authHeaders: Record<string, string>): Promise<void> {
  boundAuthHeaders = authHeaders;
  try {
