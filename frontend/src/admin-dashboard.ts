...
function effectivePaymentStatus(booking: Record<string, unknown>): string {
  const raw = String(booking.payment ?? "unpaid");
  const balance = Number(booking.balanceDueInr ?? 0);
  if (raw === "paid" && balance > 0) return "advance_paid";
  if (raw === "advance_paid" && balance <= 0 && Number(booking.packageTotalInr ?? 0) > 0) return "paid";
  return raw;
}

function bookingIsFullyPaid(booking: Record<string, unknown>): boolean {
  return effectivePaymentStatus(booking) === "paid";
}

function bookingIsAdvancePaid(booking: Record<string, unknown>): boolean {
  return effectivePaymentStatus(booking) === "advance_paid";
}

function bookingHasPaymentRecorded(booking: Record<string, unknown>): boolean {
...
function paymentBadgeClass(booking: Record<string, unknown>): string {
  if (bookingIsFullyPaid(booking)) return "badge-paid";
  if (bookingIsAdvancePaid(booking)) return "badge-advance";
  return "badge-unpaid";
}

function paymentBadgeText(booking: Record<string, unknown>): string {
  if (bookingIsFullyPaid(booking)) return "Paid";
  if (bookingIsAdvancePaid(booking)) return "Advance paid";
  return "Unpaid";
}

function bookingPaymentBreakdownHtml(booking: Record<string, unknown>): string {
  if (!bookingHasPaymentRecorded(booking) || booking.packageTotalInr == null) return "";
  return `
...
let lastTripManageRows: Array<Trip & { published?: boolean; tripStyle?: TripStyleSlug }> = [];

function tripStyleLabel(raw?: string): string {
  return TRIP_STYLES[normalizeTripStyle(raw)].shortLabel;
}

function tripStyleSelectOptions(selected?: string): string {
...
async function deleteBookingById(bookingId: string): Promise<void> {
...
async function addBookingMembers(bookingId: string, additionalTravelers: string[]): Promise<void> {
...
function openAddMembersModal(booking: Record<string, unknown>): void {
  ...
  const namesEl = wrap.querySelector("#bookingMembersNames") as HTMLElement;
  const countEl = wrap.querySelector<HTMLInputElement>('input[name="count"]');
  ...
  wrap.querySelector("#bookingMembersForm")?.addEventListener("submit", async (ev) => {
    ev.preventDefault();
    const submitBtn = wrap.querySelector<HTMLButtonElement>("#bookingMembersSubmit");
    const names = Array.from(wrap.querySelectorAll<HTMLInputElement>(".member-name-input")).map((el) =>
      el.value.trim()
    );
...
function renderTripManageList(trips: Array<Trip & { published?: boolean; tripStyle?: TripStyleSlug }>): void {
...
function openEditTripModal(trip: Trip & { published?: boolean; tripStyle?: TripStyleSlug }): void {
...
(document.getElementById("tripForm") as HTMLFormElement).addEventListener("submit", async (event) => {
  ...
  try {
    const payload = {
      ...
      published: true,
      trip_style: (document.getElementById("tripStyle") as HTMLSelectElement).value,
    };
...
(document.getElementById("bookingList") as HTMLElement).addEventListener("click", async (e) => {
  ...
  if (delBtn) {
    ...
    if (
      !window.confirm(
        "Delete this booking permanently? This cannot be undone. Use only for spam or mistaken submissions."
      )
    )
      return;
    try {
      ...
      renderBookingList(bookings);
      if (lastBookingSearch === "") renderRecentBookings(bookings);
    } catch (err) {
  ...
  if (addBtn) {
    ...
    if (booking) openAddMembersModal(booking);
    return;
  }
